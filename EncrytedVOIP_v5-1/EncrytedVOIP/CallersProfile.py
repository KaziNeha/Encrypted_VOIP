...
class PhysicalKey:
    ...
    def __init__(self, idVendor= 0x16c0, idProduct=0x05df, isInterfaceAsKeyboard = False):
        ...
        if(isInterfaceAsKeyboard == False):
            #sleep(0.1)
            jsonData = ""
            #print(self.write([10]));
    ...
    def AssignContents(self, contents):
        jsonData = ""
        StartStoring = False
        for i in contents:
            if(i is not None):
...
class PersonProfile(PhysicalKey):
    ...
    def __init__(self, isInterfaceAsKeyboard = False):
        PhysicalKey.__init__(self, isInterfaceAsKeyboard = isInterfaceAsKeyboard)
