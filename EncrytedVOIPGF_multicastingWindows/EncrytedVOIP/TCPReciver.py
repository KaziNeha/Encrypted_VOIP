...
class UDPReciever(Thread):
    ...
    def startListeningP2P(self):
            ...
            self.sock = sock
            self.ONReceptionToSpeakers = False
    ...
    def JointMulticastGroup(self, MulitcasteGrp, portNo):
        ...
        mreq = struct.pack('4s4s', group, socket.inet_aton(self.IPAddress))
        sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)
        print("[Info :] Added to the multicast group " + str(multicast_group[0]))
        self.sock = sock

        self.ConnectionEstablished = True
        self.ONReceptionToSpeakers = True
        print("Initialised")
    ...
    def run(self):
        ...
        while self.Run:
                try:
                    ...
                    if(rawSpeakerData is not None and self.ONReceptionToSpeakers == True):
